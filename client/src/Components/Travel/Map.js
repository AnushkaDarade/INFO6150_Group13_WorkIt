...
class Map extends React.Component {
	...
	getPlacesNearBySearchURL = (lat, long) => {
		return `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${long}&radius=60000&type=point_of_interest=&key=AIzaSyCEamG2dWQtxCaNKJ_xHKI7_iqtXUG9r_s`;
	}
	...
	fetchCoordinatesByPlace = (placeName) => {
		const placeGmapURL = `https://maps.googleapis.com/maps/api/geocode/json?address=${placeName}&key=AIzaSyCEamG2dWQtxCaNKJ_xHKI7_iqtXUG9r_s`;
		fetch(placeGmapURL)
			.then(res => res.json())
			...
			<div>
				...
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style={{ backgroundImage: `url(${travelImg})`, backgroundSize:'cover', WebkitBackgroundSize:`cover`,MozBackgroundSize:`cover`,oBackgroundSize: `cover`,maxHeight: `600px`,minHeight: `600px`, minWidth:`100%`}}>
					...
					<InputGroup style={{ maxWidth: `80%`, height:'25%', paddingTop:'7%', paddingLeft:'20%' }}>
						<FormControl
							placeholder="Select the place you want to check out & hit Search!"
					...
					<div className="row">
						...
						<div class="col-md-5" style={{ margin: '1%' }} >
							<AsyncMap
								...
								googleMapURL={`https://maps.googleapis.com/maps/api/js?key=AIzaSyCEamG2dWQtxCaNKJ_xHKI7_iqtXUG9r_s&libraries=places`}
								loadingElement={
